...
export default function EngineeringTeam() {
  return (
    <section className="py-24 bg-[#04060a] border-t border-white/[0.06] relative overflow-hidden" id="engineering-team">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {profiles.map((profile, i) => {
            ...
            return (
              ...
              >
                <div 
                  className="bg-white/[0.02] border border-white/[0.06] rounded-2xl p-6 hover:border-emerald-500/40 hover:shadow-2xl hover:bg-white/[0.04] transition-all duration-300 flex flex-col justify-between group backdrop-blur-xl h-full cursor-pointer"
                ...
                >
                  <div>
                    {/* Tech Symbol Avatar */}
                    <div className="w-12 h-12 rounded-xl bg-white/[0.02] border border-white/[0.08] flex items-center justify-center mb-5 font-mono text-sm font-bold text-slate-300 group-hover:bg-gradient-to-tr group-hover:from-emerald-500/10 group-hover:to-cyan-500/10 group-hover:border-emerald-500/30 group-hover:text-emerald-400 transition-all">
                      {profile.initials}
                    </div>

                    {/* Name */}
                    <h3 className="text-sm font-bold text-white tracking-tight group-hover:text-emerald-400 transition-colors font-display">
                      {profile.name}
                    </h3>
                    
                    {/* Role */}
                    <span className="text-[11px] font-semibold text-emerald-450 font-mono block mt-1">
                      {profile.role}
                    </span>

                    {/* Core Context description */}
                    <p className="text-[11px] text-slate-400 leading-relaxed mt-4 pt-4 border-t border-white/[0.06]">
